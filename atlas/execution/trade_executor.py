...
No approval needed. Real-time Telegram notification after execution.
Max 3 trades per day. Kill switch non-bypassable.
Send /pause anytime to stop ATLAS.
"""
...
from atlas.config import (
    ...
    DEFAULT_AGENT_MODE, CAPITAL_PER_TRADE
)
...
def queue_signal(signal: dict) -> dict:
    """Process signal through all checks and execute autonomously."""
    ...
    if today_count >= 3:
        log.info(f"Daily limit reached ({today_count}/3) — skipping {symbol}")
        return {"status": "SKIPPED", "reason": "Daily trade limit 3/3 reached"}
    ...
    if not is_valid:
        log.warning(f"Signal rejected — sizing: {size_reason}")
    ...
    if not ks:
        log.warning(f"Kill switch blocked: {ks.reason}")
    ...
    if current_ltp > 0:
        ...
        if direction == "LONG" and current_ltp > entry * 1.005:
            log.warning(f"Entry missed — {symbol} drifted {drift_pct:.1f}% above entry")
            return {"status": "MISSED", "reason": f"Price drifted {drift_pct:.1f}% above entry"}
        if direction == "SHORT" and current_ltp < entry * 0.995:
            log.warning(f"Entry missed — {symbol} drifted {drift_pct:.1f}% below entry")
            return {"status": "MISSED", "reason": f"Price drifted {drift_pct:.1f}% below entry"}
        entry = current_ltp
...
def _execute(signal: dict, sizing: dict, live_entry: float) -> dict:
    """Place order and manage the trade autonomously."""
    symbol    = signal.get("symbol", "")
    direction = signal.get("direction", "LONG")
    ...
    rr = round(abs(t1 - live_entry) / abs(live_entry - sl), 1) if sl and live_entry != sl else 0
    send(
...
def approve_trade(symbol: str) -> dict:
    """Kept for bot compatibility — ATLAS is autonomous."""
    return {"status": "INFO", "reason": "ATLAS is autonomous — trades execute automatically"}
