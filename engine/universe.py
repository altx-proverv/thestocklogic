...
#
# Symbols in SYMBOL_SECTOR_MAP that fail a mechanical tradeability filter.
# Generated by tools/universe_filter.py; see that file for the filters and the
# funnel. No quality judgement is involved -- a stock that cannot be exited
# during a move is not tradeable whatever its fundamentals.
#
# ONLY THE FILTERS ESTABLISHED ON CURRENT DATA ARE APPLIED HERE.
# sec_list.csv is fetched live, so series and circuit band are today's facts.
# Price, median traded value and session count come from the cached bhavcopy
# window, which on the machine this was generated from ended 2026-05-26 -- and
# "fewer than 250 sessions" is exactly the kind of statement that changes with
# the data vintage, since a stock listed in late 2025 crosses that line during
# 2026. Excluding on stale data would be asserting something not established.
#
# Re-run `python3 tools/universe_filter.py --emit-exclusions` on the box, where
# bhavcopy is current, to get the full list. That run reported a further 44
# candidates: 32 on session count, 11 on price below Rs50, 1 on liquidity.
#
# HEG and HFCL are the two worth understanding. Both are liquid, well-known
# names and both now sit in series BE with a 5% band -- NSE surveillance. A 5%
# band stock cannot be exited during a move, which is the whole reason the band
# filter exists. Their being in the universe was the bug; removing them is the
# fix.
EXCLUDED = {
    # not EQ series / not listed under this symbol (3)
    "HEG",          # series BE, band 5% -- surveillance
    "HFCL",         # series BE, band 5% -- surveillance
    "JBCHEPHARM",   # absent from sec_list.csv entirely
    # circuit band < 10% (2)
    "CEMPRO",       # band 5%
    "CPPLUS",       # band 5%
}

ALL_SYMBOLS = sorted(set(SYMBOL_SECTOR_MAP.keys()) - EXCLUDED)
